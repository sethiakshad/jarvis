...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${BACKEND_SERVER}/api/auth/login`, {
        method: 'POST',
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
      ...
      <motion.div
        ...
        style={{
          ...
          padding: '3rem 2.5rem',
          boxShadow: status === 'error' ? '0 0 30px rgba(255, 68, 68, 0.4)' : status === 'success' ? '0 0 40px rgba(0, 255, 204, 0.5)' : 'var(--glass-glow)',
          border: status === 'error' ? '1px solid var(--neon-error)' : status === 'success' ? '1px solid var(--neon-cyan)' : '1px solid var(--glass-border)',
      ...
      >
        <AnimatePresence mode="wait">
          {status === 'loading' && (
            ...
            >
              <Activity size={50} color="var(--neon-blue)" style={{ marginBottom: '1rem' }} />
              <h3 className="text-neon" style={{ color: 'var(--neon-blue)' }}>Authenticating...</h3>
              <div style={{ display: 'flex', gap: '4px', marginTop: '1rem' }}>
                {[...Array(8)].map((_, i) => (
                  <motion.div 
                    ...
                    style={{ width: '4px', background: 'var(--neon-cyan)', borderRadius: '2px' }}
                  />
                ))}
              </div>
            </motion.div>
        ...
        <motion.div variants={itemVariants} style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '1rem' }}>
            <div style={{ padding: '15px', background: 'rgba(0, 210, 255, 0.1)', borderRadius: '50%', boxShadow: '0 0 15px rgba(0, 210, 255, 0.3)' }}>
              <ShieldAlert size={32} color="var(--neon-blue)" />
            </div>
          </div>
          <h2 style={{ fontSize: '1.8rem', letterSpacing: '1px' }}>Access AI <span style={{ color: 'var(--neon-cyan)' }}>System</span></h2>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', marginTop: '0.5rem' }}>Enter credentials to initialize protocol.</p>
        </motion.div>

        <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          
          <motion.div variants={itemVariants} style={{ position: 'relative' }}>
            <div style={{ position: 'absolute', top: '50%', left: '16px', transform: 'translateY(-50%)', color: focusedId === 'id' ? 'var(--neon-cyan)' : 'var(--text-muted)', transition: 'color 0.3s' }}>
              <User size={20} />
            </div>
            <input 
              type="text" 
              ...
              style={{
                ...
                border: `1px solid ${focusedId === 'id' ? 'var(--neon-cyan)' : 'var(--glass-border)'}`,
                borderRadius: '12px', color: '#fff', fontSize: '1rem', outline: 'none',
                boxShadow: focusedId === 'id' ? '0 0 15px rgba(0, 255, 204, 0.2)' : 'none',
                transition: 'all 0.3s'
              }}
            />
          </motion.div>
          ...
          <motion.div variants={itemVariants} style={{ position: 'relative' }}>
            <div style={{ position: 'absolute', top: '50%', left: '16px', transform: 'translateY(-50%)', color: focusedId === 'pass' ? 'var(--neon-violet)' : 'var(--text-muted)', transition: 'color 0.3s' }}>
              <Lock size={20} />
            </div>
            <input 
              type="password" 
              ...
              style={{
                ...
                borderRadius: '12px', color: '#fff', fontSize: '1rem', outline: 'none',
                boxShadow: focusedId === 'pass' ? '0 0 15px rgba(138, 43, 226, 0.2)' : 'none',
                transition: 'all 0.3s'
              }}
            />
          </motion.div>
          ...
          <motion.div variants={itemVariants} style={{ marginTop: '1rem' }}>
            <motion.button 
              ...
              style={{ width: '100%', display: 'flex', justifyContent: 'center', padding: '16px', fontSize: '1.1rem', letterSpacing: '2px' }}
            >
        ...
        <motion.div variants={itemVariants} style={{ textAlign: 'center', marginTop: '2rem' }}>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>
            Unregistered Entity?{' '}
            <Link to="/register" style={{ color: 'var(--neon-blue)', textDecoration: 'none', fontWeight: 'bold' }}>
              Create Account
            </Link>
          </p>
        </motion.div>
